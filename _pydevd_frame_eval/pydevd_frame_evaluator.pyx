...
cdef class ThreadInfo:
    ...
    cdef initialize_if_possible(self, PyFrameObject * frame_obj):
        cdef str basename
        cdef str co_name
            ...
            if t is None:
                ...
                if frame_obj != NULL:
                    ...
                    if basename == 'threading' and co_name in ('__bootstrap', '_bootstrap', '__bootstrap_inner', '_bootstrap_inner'):
                        return
                    if basename == 'pydev_monkey' and co_name == '__call__':
                        return
                    if basename == 'pydevd' and co_name in ('run', 'main', '_exec'):
                        return
                    if basename == 'pydevd_tracing':
                        return

                    print('setting trace for thread started in: %s %s' % (basename, co_name))
...
cdef ThreadInfo get_thread_info(PyFrameObject * frame_obj):
    ...
    try:
        ...
        try:
            ...
            thread_info.initialize_if_possible(frame_obj)
        finally:
            thread_info.inside_frame_eval -= 1
